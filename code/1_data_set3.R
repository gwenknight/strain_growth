...
names_experiments<- c(3.1,3.2,3.3)
...
#ggsave("output/Raw_data_Reference_set3.tiff", width = 15, height = 10)
...
#ggsave("output/Raw_data_168hr drying_set3.tiff", width = 15, height = 10)


# Remove the contaminated data
# REFERENCE
w1 <- intersect(which(ddm$variable == "B2"), which(ddm$rep == 3.1))
w2 <- intersect(which(ddm$variable == "B3"), which(ddm$rep == 3.1))
w3 <- intersect(which(ddm$variable == "B4"), which(ddm$rep == 3.1))
w4 <- intersect(which(ddm$variable == "E4"), which(ddm$rep == 3.2))
#w4 <- intersect(which(grepl("E",ddm$variable)), which(ddm$rep == 23))

ddm <-ddm[-c(w1,w2,w3,w4),]

# 1wk drying
#w1 <- intersect(intersect(which(ddm$variable == "D5"), which(ddm$rep == 1.1)),which(ddm$exp == "t7"))
...
ggplot(subset(ddm,exp=="t0"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 4) + ggtitle("Reference") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_Reference_set3_tidy.tiff", width = 15, height = 10)
ggplot(subset(ddm,exp=="t7"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 4) + ggtitle("168hr drying") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_168hr drying_set3_tidy.tiff", width = 15, height = 10)
...
write.csv(ddm, "data/ddm_set3.csv")
