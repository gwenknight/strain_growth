...
cut_extract <- function(ts, Time, value, name4output, thresh_wide = 90, plot = 0, plot_where = "plots/"){
  ...
  wmax <- which.max( ts$value[-c(1:6)]) + 6 # take off funny initial behaviour < 2hr (take off in which.max and then add on 6 for position as taken 6 off)
  ...
  peaks_index = find_peaks(ts$value, m = 5)
