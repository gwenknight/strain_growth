...
names_experiments<- c(6.1,6.2,6.3)
...
#ggsave("output/Raw_data_Reference_set6.tiff", width = 15, height = 10)
...
#ggsave("output/Raw_data_168hr drying_set6.tiff", width = 15, height = 10)
...
# 1wk drying
w1 <- intersect(intersect(which(ddm$variable == "B2"), which(ddm$rep == 6.3)),which(ddm$exp == "t7"))
w2 <- intersect(intersect(which(ddm$variable == "C2"), which(ddm$rep == 6.3)),which(ddm$exp == "t7"))
...
ggplot(subset(ddm,exp=="t0"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 4) + ggtitle("Reference") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_Reference_set6_tidy.tiff", width = 15, height = 10)
ggplot(subset(ddm,exp=="t7"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 4) + ggtitle("168hr drying") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_168hr drying_set6_tidy.tiff", width = 15, height = 10)
...
write.csv(ddm, "data/ddm_set6.csv")
