...
names_experiments<- c(13.1,13.2,13.3)
...
#ggsave("output/Raw_data_Reference_set13.tiff", width = 15, height = 10)
...
#ggsave("output/Raw_data_168hr drying_set13.tiff", width = 15, height = 10)
...
ggplot(subset(ddm,exp=="t0"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 2) + ggtitle("Reference") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_Reference_set13_tidy.tiff", width = 15, height = 10)
ggplot(subset(ddm,exp=="t7"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 2) + ggtitle("168hr drying") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_168hr drying_set13_tidy.tiff", width = 15, height = 10)
...
# inoculum size
w<- which(ddm$variable == "B2"|ddm$variable == "B5")
ddm[w,"inoc"] = 5
w<- which(ddm$variable == "B3"|ddm$variable == "B6")
ddm[w,"inoc"] = 4
w<- which(ddm$variable == "B4"|ddm$variable == "B7")
ddm[w,"inoc"] = 3
...
### CHECK ALL OK? plots
ggplot(ddm, aes(x=Time,y = value, group = interaction(rep, variable, inoc, exp))) + geom_line(aes(col = factor(inoc))) + facet_wrap(drytime~strain)

#### OUTPUT
ddm <- subset(ddm, select = -c(strain_label,inoc_name)) # remove label

write.csv(ddm, "data/ddm_set13.csv")
