...
names_experiments<- c(4.1,4.2,4.3)
...
#ggsave("output/Raw_data_Reference_set4.tiff", width = 15, height = 10)
...
#ggsave("output/Raw_data_168hr drying_set4.tiff", width = 15, height = 10)


# Remove the contaminated data
# REFERENCE
w1 <- intersect(which(ddm$variable == "B2"), which(ddm$rep == 4.3))
w2 <- intersect(which(ddm$variable == "B3"), which(ddm$rep == 4.3))
w3 <- intersect(which(ddm$variable == "B5"), which(ddm$rep == 4.2))
w4 <- intersect(which(ddm$variable == "C3"), which(ddm$rep == 4.3))
#w4 <- intersect(which(grepl("E",ddm$variable)), which(ddm$rep == 23))

ddm <-ddm[-c(w1,w2,w3, w4),]

# 1wk drying
#w1 <- intersect(intersect(which(ddm$variable == "E4"), which(ddm$rep == 4.3)),which(ddm$exp == "t7"))
...
ggplot(subset(ddm,exp=="t0"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 4) + ggtitle("Reference") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_Reference_set4_tidy.tiff", width = 15, height = 10)
ggplot(subset(ddm,exp=="t7"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 4) + ggtitle("168hr drying") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_168hr drying_set4_tidy.tiff", width = 15, height = 10)
...
write.csv(ddm, "data/ddm_set4.csv")
