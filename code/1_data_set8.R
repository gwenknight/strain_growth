##### Data formatting for set8

### Automate and plot data
## libraries needed
...
names_experiments<- c(8.1,8.2,8.3)
...
#ggsave("output/Raw_data_Reference_set8.tiff", width = 15, height = 10)
...
#ggsave("output/Raw_data_168hr drying_set8.tiff", width = 15, height = 10)


# Remove the contaminated data
# REFERENCE
#w1 <- intersect(which(ddm$variable == "C4"), which(ddm$rep == 7.2))
...
# 1wk drying
w1 <- intersect(intersect(which(ddm$variable == "C4"), which(ddm$rep == 8.1)),which(ddm$exp == "t7"))
w2 <- intersect(intersect(which(ddm$variable == "C3"), which(ddm$rep == 8.1)),which(ddm$exp == "t7"))
w3 <- intersect(intersect(which(ddm$variable == "C5"), which(ddm$rep == 8.1)),which(ddm$exp == "t7"))
w4 <- intersect(intersect(which(ddm$variable == "D4"), which(ddm$rep == 8.1)),which(ddm$exp == "t7"))
w5 <- intersect(intersect(which(ddm$variable == "B4"), which(ddm$rep == 8.1)),which(ddm$exp == "t7"))
#w2 <- intersect(intersect(which(ddm$variable == "E5"), which(ddm$rep == 8.1)),which(ddm$exp == "b"))
...
ddm <-ddm[-c(w1,w2,w3,w4,w5),]

## Plot tidy data and save 
ggplot(subset(ddm,exp=="t0"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 4) + ggtitle("Reference") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_Reference_set8_tidy.tiff", width = 15, height = 10)
ggplot(subset(ddm,exp=="t7"), aes(x=Time,y=value,group = factor(rep),colour=factor(rep))) + geom_line(lwd = 1.5) + 
  facet_wrap(~variable, nrow = 4) + ggtitle("168hr drying") + scale_y_continuous("Heat (J)")
#ggsave("output/Raw_data_168hr drying_set8_tidy.tiff", width = 15, height = 10)
...
write.csv(ddm, "data/ddm_set8.csv")
