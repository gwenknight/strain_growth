...
names_experiments<- c(1.1,1.2,1.3)
...
### CHECK ALL OK? plots
ggplot(ddm, aes(x=Time,y = value)) + geom_line() + facet_wrap(drytime~strain)

#### OUTPUT
ddm <- subset(ddm, select = -c(strain_label,inoc_name)) # remove label

write.csv(ddm, "data/ddm_set1.csv")
